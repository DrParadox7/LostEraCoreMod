...
public class DynamicLightTransformer extends Transformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        ...
        for (MethodNode mn : cn.methods) {
            if (mn.name.equals("addLightSource")) {
                ...
                list.add(new MethodInsnNode(Opcodes.INVOKEINTERFACE, "atomicstryker/dynamiclights/client/IDynamicLightSource", "getAttachmentEntity", "()Lnet/minecraft/entity/Entity;", true));
                ...
                list.add(new MethodInsnNode(Opcodes.INVOKEINTERFACE, "atomicstryker/dynamiclights/client/IDynamicLightSource", "getAttachmentEntity", "()Lnet/minecraft/entity/Entity;", true));
                list.add(new FieldInsnNode(Opcodes.GETFIELD, "net/minecraft/entity/Entity", "field_70170_p", "Lnet/minecraft/world/World;"));//world
                list.add(new FieldInsnNode(Opcodes.GETFIELD, "net/minecraft/world/World", "field_73011_w", "Lnet/minecraft/world/WorldProvider;"));//worldProvider
                list.add(new FieldInsnNode(Opcodes.GETFIELD, "net/minecraft/world/WorldProvider", "field_76574_g", "I"));//dimension
    ...
    /*public static void test(IDynamicLightSource add) {
        if (add.getAttachmentEntity() != null) {
            for (int i : Pokepatch.blacklistedLightDims)
                if (add.getAttachmentEntity().worldObj.provider.dimensionId == i)
                    return;
        }
    ...
    static interface IDynamicLightSource {
